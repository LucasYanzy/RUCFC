...
export interface SpecularButtonProps {
  ...
  /* Added locally. Most of this site's calls to action navigate somewhere --
     an external form, an anchor on the page -- and a <button> that calls
     location.assign is not a link: it loses middle-click, open-in-new-tab and
     the announced role. With href set the component renders an <a> instead. */
  href?: string;
  target?: string;
  rel?: string;
}
...
const SpecularButton = ({
  ...
  if (href) {
    return (
      <a {...(rootProps as React.ComponentProps<'a'>)} href={href} target={target} rel={rel}>
        {inner}
      </a>
    );
  }
  ...
  return (
    <button {...(rootProps as React.ComponentProps<'button'>)} type={type} disabled={disabled} onClick={onClick}>
      {inner}
    </button>
  );
};
